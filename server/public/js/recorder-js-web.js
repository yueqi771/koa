...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/recorder.ts");
/******/ })
/************************************************************************/
/******/ ({
...
var Recorder = /** @class */ (function () {
    ...
    Recorder.prototype.initRecorder = function () {
        ...
        this.recorder.onaudioprocess = function (e) {
            ...
            var lData = e.inputBuffer.getChannelData(0), rData = null, vol = 0; // 音量百分比
    ...
    // getUserMedia 版本兼容
    Recorder.prototype.initUserMedia = function () {
    ...
    Recorder.prototype.getPCM = function () {
        if (this.tempPCM.length) {
            // 优先使用边录边存下的
            // 将存下的 DataView 数据合并了
            var buffer = new ArrayBuffer(this.tempPCM.length * this.tempPCM[0].byteLength), pcm_1 = new DataView(buffer), offset_1 = 0;
            // 遍历存储数据
    ...
    Recorder.prototype.getWAV = function () {
        var pcmTemp = this.getPCM(), wavTemp = Recorder.encodeWAV(pcmTemp, this.inputSampleRate, this.outputSampleRate, this.config.numChannels, this.oututSampleBits, this.littleEdian);
        return wavTemp;
    };
    /**
     * 获取WAV音频的blob数据
    ...
    Recorder.prototype.transformIntoPCM = function (lData, rData) {
        var lBuffer = new Float32Array(lData), rBuffer = new Float32Array(rData);
    ...
    Recorder.prototype.flat = function () {
        var lData = null, rData = new Float32Array(0); // 右声道默认为0
        // 创建存放数据的容器
        ...
        offset = 0;
        // 右声道
        for (var i = 0; i < this.rBuffer.length; i++) {
    ...
    Recorder.compress = function (data, inputSampleRate, outputSampleRate) {
        // 压缩，根据采样率进行压缩
        var compression = Math.max(Math.floor(inputSampleRate / outputSampleRate), 1), lData = data.left, rData = data.right, length = (lData.length + rData.length) / compression, result = new Float32Array(length), index = 0, j = 0;

(function (window) {
    ...
    var h5recorder = function (stream, config) {
        config = config || {};
    ...
    //抛出异常
    h5recorder.throwError = function (message) {
        alert(message);
        throw new function () {
    ...
    //是否支持录音
    h5recorder.canRecording = (navigator.getUserMedia != null);
    //获取录音机
    h5recorder.get = function (callback, config) {
        if (callback) {
            if (navigator.getUserMedia) {
                ...
                h5recorder.throwErr('当前浏览器不支持录音功能。');
                return;
            }
        }
    }
